...
def calculate_wind_at_height(gps_samples: List[GPSSample]) -> Tuple[float, float, float, float]:
    """
...
def wind_speed_at_height(z: float, U_H: float, H: float, z_0: float, d: float = 0.0) -> float:
    ...
    # Return zero wind speed below displacement or within roughness layer
    if effective_z <= z_0:
        return 0.0
    ...
    # U(z) = U(H) * ln((z-d)/z_0) / ln((H-d)/z_0)
    U_z = U_H * math.log(effective_z / z_0) / math.log(effective_H / z_0)
...
def calculate_cansat_landing(
    ...
    # If the wind was estimated at a reference height that is ABOVE the surface layer,
    # we scale it down to z_surface using the wind profile for the chosen terrain.
    # If H_ref <= z_surface, then U_at_surface just equals U_H scaled trivially by profile (or U_H).
    # IMPORTANT: log profile is physically intended for the surface layer only.
    # We are using it here ONLY to map wind at (approx) z_surface within that layer.
    # If H_ref is far above z_surface, consider replacing this with a measured/profiled wind at ~z_surface instead.
    if z_surface <= H_ref:
        U_at_surface = wind_speed_at_height(z=z_surface, U_H=U_H, H=z_surface, z_0=z_0, d=d)
        # The line above looks odd: reference H=z_surface makes U(z_surface)=U_H; we need a consistent choice.
        # Better: if H_ref is expected to be around z_surface, treat U_H as already at z_surface.
        # Therefore:
        U_at_surface = U_H
    else:
        # If for some reason H_ref < z_surface (rare), extrapolate upward inside surface layer
        U_at_surface = wind_speed_at_height(z=z_surface, U_H=U_H, H=H_ref, z_0=z_0, d=d)

    # Direction in surface layer is assumed same as theta from GPS wind vector
    theta_surface = theta
